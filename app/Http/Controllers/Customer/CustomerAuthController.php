...
class CustomerAuthController extends Controller
{
    ...
    {
        $customer_data = User::where('email',$email)->where('token',$token)->first();
    ...
    public function forget_password_submit(Request $request)
    {
        ...
        $customer_data = User::where('email',$request->email)->first();
        if(!$customer_data) {
    ...
    {
        $customer_data = User::where('token',$token)->where('email',$email)->first();
        if(!$customer_data) {
            return redirect()->route('customer_login');
        }
    ...
    public function reset_password_submit(Request $request)
    {
        ...
        $customer_data = User::where('token',$request->token)->where('email',$request->email)->first();
